    ...
    <section className=" py-12 lg:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <h2 className="max-w-xl font-mulish text-4xl font-extrabold leading-[1.05] text-[#101b25] sm:text-5xl">
            Frequently asked questions about Sylcchi Palace
          </h2>
          <p className="max-w-57.5 text-left font-open-sans text-sm leading-5 text-[#5b6774] md:pt-2 md:text-right">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
          {cards.map((card, index) => {
            if (card.type === "cta") {
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    className="mt-6 rounded-md bg-white px-5 py-2.5 font-mulish text-sm font-bold text-primary transition-colors hover:bg-[#eaf1f8]"
                  >
                    Ask a question
                  </button>
                </article>
            ...
            return (
              <article
                key={card.faq.q}
                className="rounded-lg bg-white p-5 shadow-[0_2px_22px_0_rgba(25,56,80,0.08)]"
              >
                <div className="mb-3 flex items-start justify-between gap-3">
                  <h4 className="font-mulish text-[32px] font-extrabold leading-[1.08] text-primary">
                    {card.faq.q}
                  </h4>
                  <ChevronDown className="mt-1 h-4 w-4 shrink-0 text-primary" />
                </div>
                <p className="font-open-sans text-sm leading-relaxed text-[#5b6774]">
                  {card.faq.a}
                </p>
              </article>
            );
