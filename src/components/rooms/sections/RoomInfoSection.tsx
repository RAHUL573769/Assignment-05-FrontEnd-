...
const RoomInfoSection = ({ room }: RoomInfoSectionProps) => {
  return (
    <div>
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-[#5b6774]">
      ...
      <p className="mt-5 font-open-sans text-base leading-relaxed text-[#5b6774]">
        {room.description}
      </p>
      <p className="mt-4 font-open-sans text-base leading-relaxed text-[#5b6774]">
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {room.facilities.map((facility) => (
          <div
            key={facility}
            className="flex items-center gap-2.5 text-[#5b6774]"
          >
